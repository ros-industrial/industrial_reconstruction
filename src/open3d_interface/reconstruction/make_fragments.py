...
from open3d_interface.utility.file import join, make_clean_folder, get_rgbd_file_lists, read_pose
from open3d_interface.utility.opencv import initialize_opencv
from open3d_interface.reconstruction.optimize_posegraph import optimize_posegraph_for_fragment

# check opencv python package
with_opencv = initialize_opencv()
if with_opencv:
    from open3d_interface.reconstruction.opencv_pose_estimation import pose_estimation
...
def register_one_rgbd_pair(s, t, color_files, depth_files, pose_files,
    ...
    # If it has tracking poses then use those as initial state
    if config["has_tracking"]:
      source_pose = read_pose(pose_files[s])
      target_pose = read_pose(pose_files[t])
      odo_init = np.linalg.inv(np.linalg.inv(source_pose).dot(target_pose))
...
def make_posegraph_for_fragment(path_dataset, sid, eid, color_files,
                                depth_files, pose_files, fragment_id,
                                n_fragments, intrinsic, with_opencv, config):
    o3d.utility.set_verbosity_level(o3d.utility.VerbosityLevel.Error)
    pose_graph = o3d.pipelines.registration.PoseGraph()
    # If it has tracking use the pose data for the initial odometry
    if config["has_tracking"]:
    ...
    for s in range(sid, eid):
        for t in range(s + 1, eid):
            ...
            if t == s + 1:
                ...
                [success, trans, info] = register_one_rgbd_pair(s, t, color_files, depth_files, pose_files, intrinsic, with_opencv, config)
...
def process_single_fragment(fragment_id, color_files, depth_files, pose_files, n_files, n_fragments, config):
    ...
    make_posegraph_for_fragment(config["path_dataset"], sid, eid, color_files,
                                depth_files, pose_files, fragment_id, n_fragments,
                                intrinsic, with_opencv, config)
...
def run(config):
    ...
    [color_files, depth_files, pose_files] = get_rgbd_file_lists(config["path_dataset"], config["has_tracking"])
    ...
    if config["python_multi_threading"] is True:
        ...
        Parallel(n_jobs=MAX_THREAD)(delayed(process_single_fragment)(
            fragment_id, color_files, depth_files, pose_files, n_files, n_fragments, config)
                                    for fragment_id in range(n_fragments))
    else:
        for fragment_id in range(n_fragments):
            process_single_fragment(fragment_id, color_files, depth_files, pose_files,
