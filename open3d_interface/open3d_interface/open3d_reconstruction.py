...
from open3d_interface_msgs.srv import RunReconstructionSystem, RunReconstructionSystemResponse
from open3d_interface_msgs.srv import RunTSDFReconstruction, RunTSDFReconstructionResponse
    ...
    for s in range(len(rgbd_array)):
        for t in range(s + 1, len(rgbd_array)):
            ...
            if t == s + 1:
                ...
                [success, trans, info] = register_known_rgbd_pair(rgbd_array[s], rgbd_array[t], odo_init, intrinsic,
                                                                  req)
...
class Open3dReconstruction(Node):
    ...
    def __init__(self):
        ...
        self.reconstruction_system_server = self.create_service(RunReconstructionSystem, 'run_reconstruction_system',
                                                                self.runReconstructionSystemCallback)
        self.tsdf_reconstruction_server = self.create_service(RunTSDFReconstruction, 'run_tsdf_reconstruction',
                                                              self.runTSDFReconstructionCallback)

    def runTSDFReconstructionCallback(self, req, res):
        ...
        for i in range(num_frames):
            ...
            rgbd = o3d.geometry.RGBDImage.create_from_color_and_depth(color, depth, req.rgbd_params.depth_scale,
                                                                      req.rgbd_params.depth_trunc,
                                                                      req.rgbd_params.convert_rgb_to_intensity)
            rgbd_array.append(rgbd)

            # Used for scene integration
            rgbd = o3d.geometry.RGBDImage.create_from_color_and_depth(color, depth, req.rgbd_params.depth_scale,
                                                                      req.rgbd_params.depth_trunc, False)
        ...
        res.success = True
        res.mesh_file = mesh_name

        return res

    def runReconstructionSystemCallback(self, req, res):
        self.get_logger().info("Reconstructing Surface")
        ...
        res.success = True
        res.mesh_file = req.path_dataset + "/scene/integrated.ply"
        return res
